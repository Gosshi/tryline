...
| **日 20:00** | [sun-recap-facts.md](sun-recap-facts.md) | 週末に終わった試合のレビュー素材 | **月 09:05** | `cron-post-match-recap-refresh` の月曜の回 |

**ChatGPT のスケジュール枠の上限に合わせて2本に統合しています**（2026-09-05）。手動で追加する枠は下記。

| 手動で回す場合 | いつ | 締切 | 使うプロンプト |
|---|---|---|---|
| 日本代表の国内戦など**JST 土曜に終わる試合** | 土 18:00頃 | **土 20:30** | `sun-recap-facts.md` を「土曜18時までに終了した試合」に読み替え |
| **JST 月曜未明の欧州戦** | 月 20:00 | **火 09:05** | `sun-recap-facts.md` に翌週の月曜 URL を追加 |
...
土曜枠は `live-pipeline` の初回生成に載せてコスト0という利点があったが、**JST 土曜18時までに終わる試合は実測で3件しかない**（14〜16時台のキックオフ）。日本代表の国内戦がある週に手動で回せば足りる。
