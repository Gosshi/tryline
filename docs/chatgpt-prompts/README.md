...
| **土 18:00** | [sat-recap-facts.md](sat-recap-facts.md) | **その日に終わった試合** | **土 20:30** | `live-pipeline` 21:00 の**初回生成**。再生成が不要でコスト0 |
...
**土曜枠だけは仕組みが違います。** 再生成ではなく `live-pipeline` の**初回生成**に載せるので、LLM コストがかかりません。ただし **JST の土曜に終わる試合がある週にしか効きません**（欧州の試合は日曜未明・月曜未明に終わる）。日本代表戦のある週が主な対象です。
