...
- **Six Nations 2027 の fixtures + results 取り込みのみ**。選手・スカッド・Reddit・他大会は対象外
...
- 仕様書「シードデータ」の SQL を新規マイグレーション `<ts>_seed_six_nations_2027.sql` として追加
...
- `tests/fixtures/wikipedia-six-nations-2027.html` として最小限の HTML サンプルを作成（実 Wikipedia の該当表のみ切り出し、`<!-- ... -->` でメモ）
