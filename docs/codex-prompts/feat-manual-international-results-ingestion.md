...
- **（2026-09-25 追記）** この仕組みで以前にスコアを入れた試合（`external_ids.result_source = "wikipedia-internationals"`）のうち、キックオフから 2 時間〜7 日で `match_events` が 0 件のものは、スコアを変えずに得点イベントだけを取り直す（仕様書の API サーフェス 1 と 4、受け入れ条件 4）。PR #889 に追加で実装する。
...
- 「壊して落ちる」確認（コミットしない）。内容と結果を PR 本文に書く。
  - ブロックを選べないときにページ全体を渡すようにすると、受け入れ条件 3 のテストが落ちる。
  - 「得点イベント待ち」の組を探す処理を外すと、受け入れ条件 4 のテストが落ちる。
