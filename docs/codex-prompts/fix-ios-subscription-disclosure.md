...
- 提供内容は「購読期間中、すべての試合のプレビュー・レビュー全文をお読みいただけます。」（**2026-08-14 変更。「AI チャット」を削除**。`tryline-mobile` に chat 機能が存在せず、Apple の Guideline 2.1 回答で矛盾するため。spec の「2026-08-14 追補」節を参照）
- **「AI チャット」の語がアプリ内のどこにも残らないこと**（`grep -rn "AI チャット" src/ app/` が0件になること）
