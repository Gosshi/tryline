...
**冪等性キーや重複排除を新設しないでください。**

**【2026-09-08 訂正】** ここには「upsert は既に冪等で、再送で壊れません」とありましたが、言い過ぎでした。**同一内容の再適用が壊れないことと、イベントの順序逆転が安全であることは別です。** 本 spec が扱うのは DB error の伝播だけで、順序制御・ゼロ行 update・欠落 metadata からの復旧は対象外です（spec の「解決しないと明示するもの」を参照）。
