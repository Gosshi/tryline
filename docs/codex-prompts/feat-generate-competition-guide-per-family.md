...
### 「伝説の名場面・選手」のリダイレクトを必ず入れる

`searchContext` は `${nameJa} ラグビー 歴史 有名試合 伝説的選手 エピソード` で Exa を検索する。**「グレイテスト・ライバルリー・ツアー」は 2026 年に付いた呼称なので、この検索はほぼ何も返さない。**

それなのにプロンプトのセクション 3 は「**年号・スコア**を盛り込んで 3〜5 つ」を要求し、厳守事項は「**参照情報に無いスコアや年号は書かない**」を要求する。**参照情報が空だと両立せず、捏造に倒れる。**

既存の `nations-championship` / `autumn-nations` は context でセクションの書き先をリダイレクトして回避している。**同じことを `greatest-rivalry` にもやること。**

context に以下に相当する 1 文を含める:
