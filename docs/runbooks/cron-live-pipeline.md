...
パイプライン自体は6時間ごとに走るが、**recap の初回生成には上限条件があり、試合終了の直後には出ない**（`specs/fix-content-generation-timing-windows.md`）。ChatGPT の調査枠（18:00・締切 20:30）を待ってから生成するため、**金曜夜の試合なら土曜 21:00 の回**になる。全体像は `docs/weekly-schedule.html` を参照。

手動実行は、その待ち時間を飛ばして目玉試合を即時に出したいときの手段。**ただし調査枠より前に回すと、事実が入らないまま確定する**点に注意する。
