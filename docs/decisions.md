...
## D025 — 日程の未取得は「節数の比較」で検出する（2026-09-01、Owner 承認済み。D024 を補完）

**背景**: `specs/fix-top14-season-fixture-coverage.md` の「未取得の可視化」が、**2つの判定方法で連続して失敗した。**

**方法1（spec 初版）: 取り込み済み最終キックオフより後の週か。** シーズンが実際に終了した大会でも成立する。**spec に懸念として書いたまま採用してしまった。**

**方法2（PR #750 の実装）: `competitions.end_date` と比較。** `lib/ingestion/live-ingest.ts:60` が `end_date: dates.at(-1)`（取り込んだ試合の最終キックオフ）としているため、**ライブ取り込み経路では `end_date === latestKickoffAt` が定義上いつでも成立**し、比較が真にならない。

本番実測:

| 大会 | `end_date` | 取込最終 | 方法2 |
|---|---|---|---|
...
PR #750 のプレビューで実際にこう出た。

> シックスネイションズ **2021**・パシフィック・ネーションズカップ 2026の日程は開催が近づいてから掲載されます。

**5年前の大会が名指しされ、主題の Top 14 は出ない。シグナルが反転していた。**

**決定**:

1. **判定を「その大会が本来持つ節数」と「取り込み済みの相異なる節数」の比較にする**
2. **`competitions` に `total_rounds integer`（nullable）を追加する。** 節を持たない大会は null のままとし、null なら注記も「全N節」も出さない
3. **`total_rounds` は取り込み処理から書き込まない。** 取り込みが基準値を上書きすることが、方法1・方法2の失敗の共通原因だった
4. 投入は3件のみ: `top-14/2026-27 = 26` / `urc/2026-27 = 18` / `premiership/2026-27 = 18`
5. **「最大の節番号」ではなく「相異なる節数」で比べる。** プレミアシップ 2026-27 は**第4節が丸ごと欠落**しており（85試合が節あり＋5試合が節情報なし）、最大値で比べると中抜けを見逃す
6. 同じ `total_rounds` を `getSeasonProgress` の分母に使い、**`/c/top-14/2026-27` の「0節 / 全2節」という誤表示も直す**（実際は全26節）
7. **PR #750 の取得窓シフトとシーズン定数の是正は問題なし。** 分離して先に扱う

**本番実測の節カバレッジ（2026-09-01）**: top-14 2節（1〜2、本来26）／urc 18節（1〜18）／premiership **17節（1〜18、第4節欠落）**／greatest-rivalry・puma-trophy・rwc・lipovitan は節を持たない。

**影響**:
- `specs/fix-schedule-coverage-notice-by-rounds.md` / 同名 codex-prompt（新規）
- `specs/fix-top14-season-fixture-coverage.md` — 注記部分を移管した旨を追記

**教訓**: **「未取得を検出する基準」を、取り込んだデータ自身から導いてはいけない。** 方法1も方法2も、基準（最終キックオフ／`end_date`）が取り込みと同じ源から来ていた。**基準は取り込みと独立した場所に置く。** 今回は `total_rounds` という別列にし、取り込みから書かないことを受け入れ条件で機械的に縛った。

**副次的な教訓**: **spec に「懸念」として書いた欠陥は、書いただけでは防げない。** 方法1の誤検知リスクは spec の未解決の質問に明記していたが、そのまま実装に進み、Codex が別案（方法2）を採ってさらに悪化した。**懸念が実装を左右するなら、未解決の質問ではなく受け入れ条件で潰す。**

## D026 — 出典 URL の存在確認は robots.txt を参照しない（2026-09-04、Owner 承認済み。CLAUDE.md のクロール規定に対する限定的な例外）

**背景**:

`specs/feat-discord-research-fact-entry.md` で、Owner が ChatGPT で調べた事実を Discord から入力する経路を設計した。**保存前に出典 URL へリクエストを投げ、200 でなければ拒否する**のがこの機能の中核である。

根拠は記録に残る失敗2回で、**どちらも失敗の形が「出典 URL が 404」だった**。

| 日付 | 事象 |
|---|---|
| 2026-07-27 | 「今週のニュース」draft 2件が捏造（URL が 404）と判明し収集停止 |
| 2026-07-29 | RUGBY DESK 案を同じリスクで見送り |

**1リクエストで両方消える。**

**対立点**:

CLAUDE.md は「robots.txt を確認せずにクロールしない」と定める。一方 `lib/scrapers/fetcher.ts` の `fetchWithPolicy` を使うと robots を強制するため、**robots で AI ボットを拒否している媒体の URL が検証段階で弾かれ、入力できなくなる。**

実測（2026-09-04）では `rnz.co.nz` と `stuff.co.nz` が ChatGPT 系を `Disallow: /`。**`nzherald.co.nz` は拒否していない**（以前3媒体まとめて「拒否」と記録していたのは誤りで、同日訂正済み）。

**決定**:

1. **素の fetch で存在確認のみ行い、robots.txt を参照しない。** `fetchWithPolicy` は使わない
2. **レスポンスボディを読まない。** HEAD を試し、405 等で拒否されたら GET にフォールバックする。リダイレクトは追い、最終ステータスが 200 なら合格
3. **タイムアウトは定数として定義する。** 200 以外・タイムアウト・接続失敗はすべて拒否し、理由を Owner に返す
4. **`source_url` にはリダイレクト後の URL ではなく入力値を保存する**
5. **この判断は spec の受け入れ条件に落とし、未解決の質問から外す**

**この例外を認める根拠**:

- **本文を1バイトも読まない。** 収集・蓄積にあたらず、リンクチェッカーと同種の振る舞いに留まる
- **Owner の明示的な操作を起点とする1回限りのリクエスト**であり、自動巡回ではない
- **Tryline 自身は何もクロールしない。** 記事を読むのは Owner 本人で、事実に著作権は無い

**この例外の範囲**:

**`app/api/discord/interactions/route.ts` の出典 URL 検証に限る。** `lib/scrapers/` 配下の取得経路、および `fetchSourcedFactsForMatch` の自動取得は従来どおり robots.txt と allowlist を厳格に適用する。**この例外を他の経路へ広げない。**

**あわせて整理し直したこと**:

`specs/feat-discord-fact-entry.md` は `news_links` 照合の必須理由を「allowlist 外ドメインの手動入力を登録できる抜け道を防ぐため」と書いていたが、**この整理は誤りだった**。allowlist は「**Tryline が自動取得してよいか**」の基準であって、「**Owner が調べた事実を入れてよいか**」の基準ではない。両者を混同していた。

Owner 以外は Discord のユーザー ID で拒否されるため、実際の論点は悪用ではなく**誤操作の防止**である。誤操作は、コンテキストメニュー（任意のメッセージに実行しうる）ではなく**明示的なスラッシュコマンド**にすることで防ぐ。**既存の通知経由の入力は変更せず、`news_links` 照合を維持する。**

**この決定が潰さないもの**:

**URL が 200 でも、その記事がその事実を書いている保証は無い。** 潰せるのは「存在しない URL」という最も粗い失敗だけで、数字・引用・負傷の詳細の正しさは Owner の目視に依存する。**この限界を承知のうえで採用する。**

**影響**:
- `specs/feat-discord-research-fact-entry.md` — 未解決の質問から外し、受け入れ条件に移す
- `docs/codex-prompts/feat-discord-research-fact-entry.md` — 承認済みである旨を明記

**教訓**: **D025 で得た「懸念が実装を左右するなら、未解決の質問ではなく受け入れ条件で潰す」がそのまま再現した。** 今回は spec に「Owner の確認が要る」と明記したため Codex が実装前に止まり、誤った実装は生まれなかった。**未解決の質問は、Codex を止める効果はあるが、止まった時点で1往復が発生する。** 判断が必要だと分かっているなら、spec を書く前に Owner に聞く。

## D027 — ニュースリンク収集と Discord 通知を停止する（2026-09-04、Owner 承認済み。2026-08-26 の設計を置き換える）

**背景**:

`cron-collect-news-links`（1日4回、上限20件）による Discord 通知が **1日13〜25件**に達し、Owner が読み切れなくなった。

この仕組みは 2026-08-26 に「RSS でリンクだけ集めて Owner が読み、事実を手で入れる」経路として作られた（[[project_news_link_pipeline]]）。**D026 の ChatGPT 調査経路がその役割を置き換えたため、維持する理由がなくなった。**

**決定的だった実測（2026-09-04）**:

**収集ソースが3つともニュージーランドの媒体だった。**
...
**欧州の媒体が1つも無い。** 2026-09-25 開幕の **URC 144試合・プレミアシップ90試合**、進行中の **Top 14** は1件も拾えない。**商品の中心である大会を、この仕組みはカバーしていなかった。**

加えて、リンク1件ごとに `translateNewsTitle`（`MODELS.FAST`）で見出しを翻訳しており、**1日4回の LLM コストが常時発生していた。**

`news_links` の読み手は2箇所だけで、**サイトには一切出ていない**（RNZ の RSS が再配信禁止のため意図的にそうしてある）。

**決定**:

1. **完全停止する。** cron・収集コード・RSS ユーティリティ・テストを削除する
2. **コンテキストメニュー経由の事実入力も引退させる。** 通知が無くなれば長押しする対象が無く、死んだ経路になる
3. **新設のスラッシュコマンド経由の入力は残す**
4. **実施の前提条件**: `specs/feat-discord-research-fact-entry.md` がマージされ、本番で1件以上の入力に成功していること。**先に停止すると事実を入れる経路が一時的にゼロになる**
5. **`news_links` テーブルは削除しない。** マイグレーションを作らない。データの削除は取り消せず、判断を覆す余地を残す
6. **`DISCORD_WEBHOOK_OPS` を消さない。** `lib/llm/notify.ts:45-53` がコンテンツ生成の失敗通知に使っている。`collect-news-links` でも使われているため「ニュース通知用」と誤認して消しやすい

**置き換え先**:

`docs/chatgpt-prompts/`（README＋時間帯別3本）。**週3回・全大会・日本語。** 締切は再生成 cron から逆算する（木21:05 / 金21:05 / 火09:05）。

**影響**:
- `specs/fix-retire-news-link-pipeline.md` / 同名 codex-prompt（新規）
- `app/api/discord/interactions/route.ts` — コンテキストメニュー経路の削除
- Owner 側: Discord のメッセージコマンド「事実を追加」の削除

**トレードオフ**:

**NZ のテストマッチ週に自動シグナルが無くなる。** ChatGPT 経路は週3回なので、その間に出た情報は次の回まで拾われない。**速報性は元々捨てている**（`x-post` スキル）ので許容する。

**教訓**: **仕組みを作る前に、それが商品の中心をカバーするかを確かめる。** この仕組みは稼働から9日で役目を終えたが、根本原因は「使えた RSS が3つとも NZ だった」ことを**採用時に方針として扱わなかった**こと。**取れるソースから設計を始めると、必要なカバレッジと噛み合わない。**

## D028 — 日本代表戦の結果は JRFU から補完する（2026-09-07、実装完了。5PR を要した経緯と失敗の記録を含む）

**背景**:

2026-09-05 の日本×カナダは、試合終了から**28時間**経ってもレビューが生成されなかった。

取り込み元の ja.wikipedia が未記入（`- - 前半-`）で `status` が `scheduled` のままだったため、`lib/cron/orchestrate.ts` のレビュー生成（`status = "finished"` の試合のみ対象）が動かなかった。

**JRFU の日程ページ（`rugby-japan.jp/schedule/`）には 57–12 が出ており、試合ページ（`/match/29975`）には得点経過もあった。** 日程ページは既に放送情報の取り込みで使っていたが、パーサは `matchUrls` と `year` しか見ていなかった。

**決定**:

1. **スコアは日程ページから補完する**（#776）。試合ごとに記事 URL をハードコードする従来の回避策（`JAPAN_FIJI_JRFU_URL`）は記事 ID が予測できないため採らない
2. **得点経過は試合ページから補完する**（#778）。`icon-try` / `icon-conversion` / `icon-pg` / `icon-dg` で種別を判別する
3. **後半の分数に +40 のオフセットを加える**（#782）。JRFU は前半・後半で分数を振り直している
4. **単調性はハーフごとに検証する**（#783）。境界は検証しない
5. **選手は英語名で引けなければ日本語表記でも引く**（#784）。空白を除去した完全一致
6. **既存の結果・イベントを上書きしない。** 補完に限る
7. **`fetchWithPolicy` を使う。** D026 の robots 例外はここには適用しない

**副次的に判明したこと**:

- **`match_events` の type は `try` / `conversion` / `penalty_goal` / `drop_goal` の4種のみ。カードは存在しない**
- **`match_team_stats` は全試合0行**
- **`players.name_ja` は日本81人中11人・カナダ0人**（全体では2456人中23人）

## この機能に5PR を要した理由（Claude の失敗の記録）

**spec の欠陥が4回、git の事故が2回あった。いずれも Claude Code の責任である。**

| 回 | 欠陥 | 検出時点 |
|---|---|---|
| 1 | 「JRFU の日付は JST」と書いた。**実際は現地時間表示**（ページに明記あり） | Codex が実装前に停止 |
| 2 | 投入条件を「イベント合計とスコアの一致」だけにした。**合計が合っても時系列は壊れうる** | **本番で誤った記事が公開された** |
| 3 | 「オフセット適用後に全体で単調非減少」と書いた。**前半ロスタイムを加算しない条件と両立しない** | 投入前に停止 |
| 4 | 選手解決の前提を確認しなかった。**`resolvePlayerId` は英語名しか見ていない** | 投入前に停止 |

**4回とも原因は同じ**である。**依存する既存関数と外部ページを最後まで読まずに条件を書いた。** JRFU が現地時間であること、`resolvePlayerId` が `players.name` だけを見ること、いずれも数分の確認で分かった。

**3 は特に悪い。** フィクスチャの分数（`前半 … 42' 43'` / `後半 1' 2' …`）を自分で出力させた直後に、そのデータが満たさない条件を書いている。**情報が無かったのではなく、持っている情報と書いた条件を突き合わせなかった。**

### git の事故

**`git reset --soft origin/main` を使い、インデックスが古いまま残った状態でコミットした。** 結果、コミットが古いツリー全体を書き戻した。

| コミット | 巻き添え |
|---|---|
| `11ddd95` | `tools/audit-published-recap-event-integrity.ts`（873行）とテストを削除。別セッションが `e4a6256` で復旧 |
| `c0ca929` | **43ファイル・1852行**を巻き戻し。#782 の実装、#781 のテスト、Stripe/料金/特商法、runbook、`scripts/backfill-*` 8本 |

**`c0ca929` の間に定期実行が2本走り、古いパーサが誤ったイベントを再投入し、月曜のレビュー再生成が誤った記事を再公開した。**

**1度目の事故を把握していながら同じ手順を使い続けた点で、2度目は質が違う。**

**教訓**:

1. **spec を書く前に、依存する関数の本体と外部ページを開いて引用する。** 「たぶんこうなっている」で条件を書かない
2. **検証条件を書いたら、手元の実データに当ててから確定する。** 合計・単調性のような条件は、実際の値の列に適用して確かめる
3. **`git reset --soft` を使わない。** コミットは `origin/main` から切った一時 worktree の中だけで作り、push 前に `git diff --cached --name-only` で対象を目視する
4. **「合計が合う」は「中身が正しい」ではない。** 検算を設計するときは「この検算をすり抜ける壊れ方は何か」を先に考える

**影響**:
- `specs/feat-jrfu-result-fallback.md` / `feat-jrfu-match-event-fallback.md` / `fix-jrfu-event-half-offset.md` / `fix-player-resolution-by-japanese-name.md`
- PR #776 / #778 / #782 / #783 / #784

## D029 — 手動入力の事実を生成に優先して渡す（2026-09-07、Owner 承認済み。D026 の運用を機能させるため）

**背景**:

**Owner が調査して入れた事実24件のうち、生成に渡ったのは1件だけだった。**

`loadSourcedFactsForMatch`（`lib/llm/sourced-facts/fetch.ts:383`）は `fetched_at` の降順で `MAX_STORED_FACTS = 8` 件しか取らない。**`fetch-sourced-facts?force=true` が自動取得を毎回取り直すため、自動側の `fetched_at` が実行時刻に更新され、手動を押し出す。**

手動は一度入れたら `fetched_at` が動かない（自動再取得の削除対象から除外されている）。**再生成するほど手動が押し出される構造になっていた。**

落ちたのは、エディー・ジョーンズのコメント、稲場巧の初トライの本人談、**公式 Player of the Match が確認できなかったという記録**、モール／スクラム／ラインアウトの文脈。**`docs/chatgpt-prompts/` で「自動で取れない層を優先して集めよ」と指示している、まさにその層である。**

**決定**:

1. **手動（`metadata.entry_method === "manual"`）を先に選ぶ。** 枠が余ったら自動で埋める
2. **手動が8件以上あれば自動は渡らない。** Owner が調査して入れたものを優先する
3. **`MAX_STORED_FACTS` は 8 のまま。** プロンプト長・コスト・QA 字数条件への影響を測っていないため変えない
4. **allowlist フィルタを上限適用より前に移す。** 現状は除外された分だけ枠が空のまま渡っており、8枠が8件で埋まっていなかった

**影響**:
- `specs/fix-manual-facts-priority-in-selection.md` / 同名 codex-prompt
...
## D031 — 手動入力の事実は8件を超えても生成に渡す（2026-09-12、Owner 承認済み。D029 の決定3を改める）

**背景**:

2026-09-12 の日本×アメリカで、アメリカの先発発表を手動で9件追加したところ、**同じ試合の手動事実が16件になり、新しい8件だけが生成に渡る状態になった。** 日本側の先発・対戦成績・キックオフの7件がすべて落ちるところだった。Owner が SQL で8件に統合して回避した。

D029 は「プロンプト長・コスト・QA 字数条件への影響を測っていない」ことを理由に上限 8 を据え置いた。**今回その影響を実測した。**

| 項目 | 実測（2026-09-12） |
|---|---|
| 手動事実がある 試合×種別 | 21組。うち8件超が3組、16件超が1組（最大17件） |
| 手動事実1件あたりのプロンプト上の長さ | JSON で平均278字 |
| 9/12 の7試合を再生成した LLM 費用 | 合計 $0.4266（1試合 $0.0364〜0.0848） |
| 手動を8件増やしたときの上乗せ（上限見積もり） | 1試合あたり約 $0.011（ナラティブ2回・QA 2回に約2,224字が乗る場合） |

**決定**:

1. **手動事実は最大16件まで生成に渡す**（`MAX_MANUAL_FACTS_FOR_GENERATION = 16`）
2. **自動取得の事実は、手動が8件未満のときだけ、合計8件になるまで埋める。** 手動が8件以下の試合の挙動は今と変わらない
3. **16件を超えた分と、自動が締め出されることは、Discord の保存応答で Owner に知らせる。** 黙って落とさない
4. **キャッシュ判定は生成用の上限と切り離す。** 手動が8件以上ある試合で、自動取得のキャッシュが無いと誤判定され、毎回 Web 検索をやり直していた（同時に直す）

**影響**: `specs/fix-manual-facts-generation-cap.md` / 同名 codex-prompt

## D032 — 出典 URL がボット拒否（403/429）を返すときは、Owner の明示操作で保存を許す（2026-09-12、Owner 承認済み。D026 の決定3を改める）

**背景**:

2026-09-12、米国代表の公式発表（`eagles.rugby`）を出典にした事実が、Discord で「出典 URL が HTTP 429 を返しました」と拒否された。**ページは実在し、Owner が目視で内容を確認していた。** 同サイトは robots.txt にも 403 を返すボット拒否で、Owner が SQL で直接入れて回避した。

**403/429 はボット対策が URL の実在と無関係に返すため、存在確認として機能しない。** 一方、D026 が防ぎたかった失敗は「404 の捏造 URL」である。

**決定**:

1. **403 と 429 に限り、Owner がモーダルで「目視確認済み」を選んだときだけ保存する。** 既定は従来どおり拒否し、選び方を案内する
2. **404・その他の 4xx/5xx・タイムアウト・接続失敗は、目視確認済みを選んでも拒否する**
3. **保存した事実の `metadata` に、未検証のまま Owner 判断で保存したことと HTTP ステータスを残す**
4. D026 の他の決定（robots.txt を参照しない・本文を読まない・入力値を保存する）は変えない

**この決定が潰さないもの**: 403/429 のサイトでは、URL の打ち間違いを機械的に検出できなくなる。**Owner がリンクを開いて確認したことが唯一の担保になる。**

**影響**: `specs/feat-discord-source-url-owner-verified.md` / 同名 codex-prompt。モーダルのコンポーネントが上限の5個に達する

## D033 — 出典 URL の 401 も Owner の目視確認で保存を許す（2026-09-12、Owner 承認済み。D032 の決定1・2 を広げる）

**背景**:

同じ 2026-09-12、Reuters の記事（`https://www.reuters.com/sports/springboks-make-two-changes-starting-xv-final-new-zealand-test-2026-09-07/`）を出典にした南アフリカ×ニュージーランドの事実が、Discord で **HTTP 401** を理由に拒否された。**ページは実在し、Owner が目視で内容を確認していた。** D032 の実装（PR #815）は許容集合を `{403, 429}` としていたため、「目視で確認済み」を選んでも 401 は保存できなかった。

**401 も 403/429 と同じく、ボット判定や購読判定が URL の実在と無関係に返す。** D026 が防ぎたかった「404 の捏造 URL」とは別物である。

**決定**:

1. **401 を `OWNER_VERIFIABLE_SOURCE_URL_STATUSES` に加える。** 403/429 と同じ扱いで、Owner がモーダルで「目視確認済み」を選んだときだけ保存する
2. **404・401/403/429 以外の 4xx/5xx・タイムアウト・接続失敗は、引き続き目視確認済みを選んでも拒否する**
3. D032 の決定3・4（`metadata` に HTTP ステータスを残す / D026 の他の決定は変えない）はそのまま適用する
