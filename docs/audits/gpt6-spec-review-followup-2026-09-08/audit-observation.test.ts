...
vi.mock("@/lib/db/server", () => ({
  getSupabaseServerClient: () => {
    throw new Error("No real DB access");
  },
}));
...
it("anonymous unrelated logs stay suspect even when one score is incomplete", async () => {
  const db = dbFor({
    match_content: [
      {
        match_id: "target",
    ...
    matches: ["target", "other"].map((id) => ({
      ...
      status: "finished",
      external_ids: {},
      kickoff_at: "2026-09-08T00:00:00Z",
      home_team: { name: "Home" },
      away_team: { name: "Away" },
      competition: { slug: "synthetic" },
    })),
    match_events: ["target", "other"].flatMap((id) =>
      Array.from({ length: 4 }, (_, index) => ({
        match_id: id,
        minute: index + 1,
        type: "try",
        metadata: {},
        team_id: id === "target" ? "home" : "away",
      })),
    ),
  });
...
it("partial standings across competitions stay incomplete and retain schedule teams", async () => {
  const report = await auditCompetitionGuideFacts(
    dbFor({
      ...
      competition_teams: [{ competition_id: "pool-a", team_id: "a" }],
      matches: [
        { competition_id: "pool-b", home_team_id: "b", away_team_id: "c" },
      ],
      teams: ["a", "b", "c"].map((id) => ({
        id,
    ...
    { family: "synthetic", season: "2026", outputDir: "unused" },
    "2026-09-08T00:00:00Z",
  );
  expect(report.coverage).toBe("incomplete");
  expect(report.actualDataTeams).toHaveLength(3);
  expect(report.guideOnlyCandidates).toHaveLength(0);
});
