...
it("preserves a JRFU rejection through a later fetch failure and returns HTTP 500", async () => {
  ...
  expect(body.rejections).toEqual([expect.objectContaining({ matchId: "synthetic-0", reason: "fixture_conflict" })]);
  expect(
    body.results.some(
