...
const FAMILY = "autumn-nations";

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}$/.test(value)) {
    console.error(
      "Usage: pnpm tsx scripts/import-autumn-nations-results.ts <YYYY>",
    );
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive Autumn Nations competition dates.");
  }
...
async function main() {
  ...
  const results =
    await wikipediaAutumnNationsResultsScraper.fetchResults(season);

  if (results.length === 0) {
    console.error(`No finished Autumn Nations ${season} matches found.`);
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for Autumn Nations ${season}`,
  );
}
