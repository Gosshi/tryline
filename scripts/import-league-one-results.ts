...
const EXPECTED_MIN_MATCH_COUNT = 5;

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}-\d{2}$/.test(value)) {
    console.error(
      "Usage: pnpm tsx scripts/import-league-one-results.ts <YYYY-YY>",
    );
    process.exit(1);
  }
...
async function main() {
  ...
  const results = await wikipediaLeagueOneResultsScraper.fetchResults(season);

  if (results.length < EXPECTED_MIN_MATCH_COUNT) {
    console.error(
      `Expected at least ${EXPECTED_MIN_MATCH_COUNT} finished League One ${season} playoff matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for League One ${season}`,
  );
}
