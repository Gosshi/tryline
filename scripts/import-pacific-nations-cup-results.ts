...
const FAMILY = "pacific-nations-cup";
const EXPECTED_MATCH_COUNT = 11;

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}$/.test(value)) {
    console.error(
      "Usage: pnpm tsx scripts/import-pacific-nations-cup-results.ts <YYYY>",
    );
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive Pacific Nations Cup competition dates.");
  }
    ...
    .upsert(
      {
        end_date: endDate,
        family: FAMILY,
        name: `Pacific Nations Cup ${season}`,
        season,
        slug: `${FAMILY}-${season}`,
...
async function main() {
  ...
  const results =
    await wikipediaPacificNationsCupResultsScraper.fetchResults(season);

  if (results.length !== EXPECTED_MATCH_COUNT) {
    console.error(
      `Expected ${EXPECTED_MATCH_COUNT} finished Pacific Nations Cup ${season} matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for Pacific Nations Cup ${season}`,
  );
}
