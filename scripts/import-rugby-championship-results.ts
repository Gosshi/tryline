...
const FAMILY = "rugby-championship";
const EXPECTED_MATCH_COUNT = 12;

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}$/.test(value)) {
    console.error(
      "Usage: pnpm tsx scripts/import-rugby-championship-results.ts <YYYY>",
    );
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive Rugby Championship competition dates.");
  }
    ...
    .upsert(
      {
        end_date: endDate,
        family: FAMILY,
        name: `Rugby Championship ${season}`,
        season,
        slug: `${FAMILY}-${season}`,
...
async function main() {
  ...
  const results =
    await wikipediaRugbyChampionshipResultsScraper.fetchResults(season);

  if (results.length !== EXPECTED_MATCH_COUNT) {
    console.error(
      `Expected ${EXPECTED_MATCH_COUNT} finished Rugby Championship ${season} matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for Rugby Championship ${season}`,
  );
}
