...
const FAMILY = "super-rugby-pacific";
const EXPECTED_MATCH_COUNT = 83;

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}$/.test(value)) {
    console.error(
      "Usage: pnpm tsx scripts/import-super-rugby-pacific-results.ts <YYYY>",
    );
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive Super Rugby Pacific competition dates.");
  }
    ...
    .upsert(
      {
        end_date: endDate,
        family: FAMILY,
        name: `Super Rugby Pacific ${season}`,
        season,
        slug: `${FAMILY}-${season}`,
...
async function main() {
  ...
  const results =
    await wikipediaSuperRugbyPacificResultsScraper.fetchResults(season);

  if (results.length !== EXPECTED_MATCH_COUNT) {
    console.error(
      `Expected ${EXPECTED_MATCH_COUNT} finished Super Rugby Pacific ${season} matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for Super Rugby Pacific ${season}`,
  );
}
