...
const FAMILY = "urc";
const EXPECTED_MATCH_COUNT = 7;

function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}-\d{2}$/.test(value)) {
    console.error("Usage: pnpm tsx scripts/import-urc-results.ts <YYYY-YY>");
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive URC competition dates.");
  }
    ...
    .upsert(
      {
        end_date: endDate,
        family: FAMILY,
        name: `United Rugby Championship ${season}`,
        season,
        slug: `${FAMILY}-${season}`,
...
async function main() {
  ...
  const results = await wikipediaUrcResultsScraper.fetchResults(season);

  if (results.length !== EXPECTED_MATCH_COUNT) {
    console.error(
      `Expected ${EXPECTED_MATCH_COUNT} finished URC ${season} knockout matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for URC ${season}`,
  );
}
