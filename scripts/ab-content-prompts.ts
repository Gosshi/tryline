...
export function parsePromptExperimentArgs(argv: string[]): Args {
  ...
  const variants = (values.get("--variants") ?? "A,B")
    .split(",")
    .map((value) => value.trim());
  if (
    variants.length === 0 ||
    variants.some((value) => value !== "A" && value !== "B") ||
    new Set(variants).size !== variants.length
  ) {
    throw new Error("--variants must be A, B, or A,B");
  }
  const pairWith = values.get("--pair-with");
  if (pairWith && command !== "run")
    throw new Error("--pair-with is only supported for run");
  if (pairWith && (variants.length !== 1 || variants[0] !== "B")) {
    throw new Error("--pair-with requires --variants B");
  }
  const maxUsd = Number(values.get("--max-usd") ?? 2);
  if (
    command === "freeze" &&
    (!matches?.length || (contentType !== "preview" && contentType !== "recap"))
  ) {
    throw new Error(
      "freeze requires --matches <id,...> and --content-type preview|recap",
    );
  }
  if (command === "run" && !fixtures?.length)
    throw new Error("run requires --fixtures <file,...>");
  ...
  return {
    ...
    variants: variants as Array<"A" | "B">,
    ...(pairWith ? { pairWith } : {}),
  };
}
...
export async function freezePromptFixtures(
  ...
  if (!args.matches?.length || !args.contentType)
    throw new Error("freeze arguments are incomplete");
  ...
  for (const matchId of args.matches) {
    try {
      ...
      if (
        args.contentType === "preview" &&
        (assembled.match.status !== "scheduled" ||
          assembled.match.home_score !== null ||
          assembled.match.away_score !== null)
      ) {
        throw new Error(
          "preview freeze requires a scheduled match with no score",
        );
      }
      if (
        args.contentType === "recap" &&
        (assembled.match_events.length === 0 ||
          assembled.eventIntegrity.status === "mismatch")
      ) {
        throw new Error(
          "recap freeze requires match events and matching score integrity",
        );
      }
      ...
      const path = join(
        outputDirectory,
        `${safePart(matchId)}-${args.contentType}.json`,
      );
...
export async function runPromptExperiment(
  args: Pick<Args, "fixtures" | "maxUsd" | "dryRun" | "reveal"> &
    Partial<Pick<Args, "variants" | "pairWith">>,
  dependencies: RunDependencies,
) {
  const fixtures = await Promise.all(
    (args.fixtures ?? []).map(async (path) => ({
      path,
      fixture: JSON.parse(
        await dependencies.read(path, "utf8"),
      ) as FrozenFixture,
    })),
  );
  if (fixtures.length === 0) throw new Error("No fixtures supplied");
  const requestedVariants = args.variants ?? ["A", "B"];
  if (
    args.pairWith &&
    (requestedVariants.length !== 1 || requestedVariants[0] !== "B")
  ) {
    throw new Error("--pair-with requires --variants B");
  }
  ...
  if (args.pairWith) {
    const ledgerText = await dependencies.read(
      join(args.pairWith, "ledger.json"),
      "utf8",
    );
    const blindKeyText = await dependencies.read(
      join(args.pairWith, "blind-key.json"),
      "utf8",
    );
    const priorLedger = JSON.parse(ledgerText) as Array<{
      matchId?: unknown;
      variant?: unknown;
    }>;
    const priorBlindKey = JSON.parse(blindKeyText) as Record<
      string,
      Partial<Record<"X" | "Y", unknown>>
    >;
    if (
      !Array.isArray(priorLedger) ||
      !priorBlindKey ||
      typeof priorBlindKey !== "object"
    ) {
      throw new Error(
        "--pair-with requires a valid ledger.json and blind-key.json",
      );
    }
    for (const { fixture } of fixtures) {
      const matchId = fixture.assembled.match.id;
      const hasPriorB = priorLedger.some(
        (row) => row.matchId === matchId && row.variant === "B",
      );
      const key = priorBlindKey[matchId];
      const blindSlot = key?.X === "B" ? "X" : key?.Y === "B" ? "Y" : null;
      if (!hasPriorB || !blindSlot) {
        warn(
          `[pair-with] No previous B article for ${matchId}; B2 will not be paired.`,
        );
        continue;
      }
      try {
        const article = await dependencies.read(
          join(args.pairWith, "blind", `${safePart(matchId)}-${blindSlot}.md`),
          "utf8",
        );
        if (!article.trim()) {
          warn(
            `[pair-with] Previous B article is empty for ${matchId}; B2 will not be paired.`,
          );
          continue;
        }
        previousB.set(matchId, article);
      } catch (error) {
        if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
        warn(
          `[pair-with] Previous B article is missing for ${matchId}; B2 will not be paired.`,
        );
      }
    }
  }

  const contentTypeFor = (path: string): ContentType =>
    path.endsWith("-preview.json") ? "preview" : "recap";
  const averages = new Map<ContentType, number>();
  for (const type of new Set(
    fixtures.map(({ path }) => contentTypeFor(path)),
  )) {
    averages.set(type, await dependencies.averageCost(type));
  }
  const estimate = fixtures.reduce(
    (sum, row) =>
      sum +
      (averages.get(contentTypeFor(row.path)) ?? 0) * requestedVariants.length,
    0,
  );
  const random = seededRandom(dependencies.seed);
  const plans = fixtures.map(({ path, fixture }) => {
    const variants =
      requestedVariants.length === 2
        ? random() < 0.5
          ? (["A", "B"] as const)
          : (["B", "A"] as const)
        : [requestedVariants[0]!];
  ...
  if (args.dryRun) {
    const variantLabel = requestedVariants.join(",");
    return {
      estimateUsd: estimate,
      averageCostUsd: Object.fromEntries(averages),
      plans: plans.map(({ matchId, contentType: type, variants }) => ({
        matchId,
        contentType: type,
        variants,
      })),
      output: `[dry-run] ${plans.length} fixtures × ${requestedVariants.length} variant(s) (${variantLabel}); estimate $${estimate.toFixed(4)}. No LLM calls or artifacts created.`,
    };
  }
  ...
  const outputDirectory = join(
    ...
    dependencies.now().toISOString().replace(/[:.]/g, "-"),
  );
  await dependencies.mkdir(join(outputDirectory, "prompts"), {
    recursive: true,
  });
  await dependencies.mkdir(join(outputDirectory, "articles"), {
    recursive: true,
  });
  const writesBlindArtifacts =
    requestedVariants.length === 2 || Boolean(args.pairWith);
  if (writesBlindArtifacts)
    await dependencies.mkdir(join(outputDirectory, "blind"), {
      recursive: true,
    });
  const ledger: Array<Record<string, unknown>> = [];
  const blindKey: Record<
    string,
    Partial<Record<"X" | "Y", "A" | "B" | "B1" | "B2">>
  > = {};
  let spentUsd = 0;
  ...
  for (const plan of plans) {
    let completed = 0;
    let currentB2: string | null = null;
    for (const variant of plan.variants) {
      ...
      spentUsd += costUsd;
      const recapLengths =
        plan.contentType === "recap"
          ? (() => {
              const split = splitRecapForPaywall(details.content);
              return {
                freeCharacters: split.freeMd.length,
                paidCharacters: split.lockedMd?.length ?? 0,
              };
            })()
          : {};
      ledger.push({
        ...
        models: Object.fromEntries([
          ...
          ...details.stageMetrics.map((stage) => [
            stage.name,
            stage.modelVersion,
          ]),
        ]),
      ...
      await dependencies.write(
        join(
          outputDirectory,
          "prompts",
          `${safePart(plan.matchId)}-${variant}.txt`,
        ),
        details.prompt,
        "utf8",
      );
      await dependencies.write(
        join(
          outputDirectory,
          "articles",
          `${safePart(plan.matchId)}-${variant}.md`,
        ),
        details.content,
        "utf8",
      );
      if (variant === "B") currentB2 = details.content;
      if (requestedVariants.length === 2) {
        ...
        await dependencies.write(
          join(
            outputDirectory,
            "blind",
            `${safePart(plan.matchId)}-${blindSlot}.md`,
          ),
          details.content,
          "utf8",
        );
    ...
    if (requestedVariants.length === 2 && completed === 2) {
      const xVariant = plan.xIsA ? "A" : "B";
      const yVariant = xVariant === "A" ? "B" : "A";
      blindKey[plan.matchId] = { X: xVariant, Y: yVariant };
    } else if (args.pairWith && currentB2) {
      const priorArticle = previousB.get(plan.matchId);
      if (priorArticle) {
        const b2IsX = random() < 0.5;
        const xBody = b2IsX ? currentB2 : priorArticle;
        const yBody = b2IsX ? priorArticle : currentB2;
        blindKey[plan.matchId] = b2IsX
          ? { X: "B2", Y: "B1" }
          : { X: "B1", Y: "B2" };
        await dependencies.write(
          join(outputDirectory, "blind", `${safePart(plan.matchId)}-X.md`),
          xBody,
          "utf8",
        );
        await dependencies.write(
          join(outputDirectory, "blind", `${safePart(plan.matchId)}-Y.md`),
          yBody,
          "utf8",
        );
      }
    }
    if (completed !== plan.variants.length) break;
    if (haltedByBudget) break;
  }

  await dependencies.write(
    join(outputDirectory, "ledger.json"),
    `${JSON.stringify(ledger, null, 2)}\n`,
    "utf8",
  );
  if (writesBlindArtifacts)
    await dependencies.write(
      join(outputDirectory, "blind-key.json"),
      `${JSON.stringify(blindKey, null, 2)}\n`,
      "utf8",
    );
  const reviewMatchIds = Object.keys(blindKey);
  const reviewSheet = [
    "# 読み比べ評価シート",
    ...reviewMatchIds.flatMap((matchId) => [
      "",
      `## ${matchId}`,
      "",
      "| 評価項目 | X | Y | 同 |",
  ...
  await dependencies.write(
    join(outputDirectory, "review-sheet.md"),
    `${reviewSheet}\n`,
    "utf8",
  );
  const summaryLabels: string[] = args.pairWith
    ? ["B2"]
    : requestedVariants.length === 2 && !args.reveal
      ? ["X", "Y"]
      : requestedVariants;
  const summaryRows = summaryLabels.map((label) => {
    const rows = args.pairWith
      ? ledger
      : requestedVariants.length === 2
        ? ledger.filter(
            (row) =>
              blindSummaryLabel(
                row.variant as "A" | "B",
                blindKey[String(row.matchId)]?.X === "A",
                args.reveal,
              ) === label,
          )
        : ledger.filter((row) => row.variant === label);
    const average = (key: string) =>
      rows.length
        ? (
            rows.reduce(
              (sum, row) =>
                sum +
                Number(
                  (row.qa as { scores: Record<string, number> }).scores[key],
                ),
              0,
            ) / rows.length
          ).toFixed(2)
        : "—";
    const verdicts =
      rows.map((row) => (row.qa as { verdict: string }).verdict).join(", ") ||
      "—";
  ...
  const summary = [
    ...
    `- Random seed: ${dependencies.seed}`,
    `- Cost estimate: $${estimate.toFixed(4)}`,
    `- Actual cost: $${spentUsd.toFixed(4)}`,
    ...(args.pairWith
      ? [
          "- QA: B2 の結果のみを表示。前回の B1 とは QA コードが異なる可能性があるため、点数を比較しません。",
        ]
      : []),
    ...(haltedByBudget
      ? [
          "- Stopped before the next call because the cost ceiling would be exceeded.",
        ]
      : []),
    "",
...
async function main() {
  ...
  const result = await runPromptExperiment(args, {
    generate: async (matchId, contentType, variant, frozenInput) =>
  ...
  console.log(
    result.output ??
      `Wrote ${result.ledger.length} trial rows to ${result.outputDirectory}`,
  );
}
