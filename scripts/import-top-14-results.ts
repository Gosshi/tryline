...
function parseSeasonArg(value: string | undefined) {
  if (!value || !/^\d{4}-\d{2}$/.test(value)) {
    console.error("Usage: pnpm tsx scripts/import-top-14-results.ts <YYYY-YY>");
    process.exit(1);
  }
...
function getCompetitionDates(results: HistoricalMatchResult[]) {
  ...
  if (!startDate || !endDate) {
    throw new Error("Unable to derive Top 14 competition dates.");
  }
...
async function main() {
  ...
  if (results.length < EXPECTED_MIN_MATCH_COUNT) {
    console.error(
      `Expected at least ${EXPECTED_MIN_MATCH_COUNT} finished Top 14 ${season} playoff matches, got ${results.length}.`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `Upserted ${upsertedCount} matches and ${teamCount} competition_teams for Top 14 ${season}`,
  );
}
