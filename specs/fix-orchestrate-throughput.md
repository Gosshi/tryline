...
- **応答は `NextResponse.json(result)` で、`OrchestrateResult` を JSON の直下に返している**（`app/api/cron/orchestrate/route.ts:88`、`apiSuccess` の包み方ではない）。`remaining` は **`.remaining`** から読む。**応答の形は変えない**（2026-09-24 訂正。当初 `.data.remaining` と書いたのは誤り）
- **応答に `remaining` が無ければ、ループを止めてステップを失敗にする。** route は内部で例外が起きると、0 件の結果を**ステータス 200** で返す（`route.ts:94-110`）ため、今は `curl -f` で例外を検知できない。この応答には `remaining` が入らないので、これで例外に気づけるようになる。route の例外時の応答そのものは変えない
...
7. **ワークフロー**: `remaining` が 0 のとき 1 回で終わり、残りがあるときは最大 4 回で止まり、**応答に `remaining` が無いとき（例外時の 200 応答）は失敗で止まる**ことを、ループ部分をスクリプト化するなどしてテストする（YAML の中に長いシェルを直書きしない。`scripts/` か `tools/` に置いて単体テストする）
