...
5. V3: 4 件以上の署名列が別 match_id と完全一致する場合に**警告（`warnings`）が出て、取り込みは続行される**ことを検証するテストがある。**3 件以下では警告も出ない。**

   **2026-09-08 訂正**: 初版は「拒否され」としていたが、2026-09-06 の Owner 判断（未解決の質問 2）で **V3 は警告のみ・取り込みを拒否しない**と確定した。実装（PR #781）も `warnings` に入れており `rejected` には入れない。**この AC が旧のままだと、次に読む実装者が V3 を拒否へ戻す。**
...
- **V3 と V4 は同一ラン内で新たに作られた重複を検出できない。** `getEventInsertionValidationSnapshot()` は他試合の署名と fixture 識別子の対応表を 1 回だけ読み、insert 後に更新しない。2026-07-06 の NC 第1節（6 試合が 1 回の取り込みで同じ 18 イベントを共有）のような形では、2 件目以降のコピーがスナップショットの何とも一致せず警告が出ない。

  **2026-09-08 訂正**: 初版は「V1 / V2 / V4 は毎回 canonical な試合データを引くため影響を受けない」と書いたが、**V4 については誤り**。V4 が照合する fixture 識別子の**対応表そのものがキャッシュされている**ため、対象試合だけを毎回読み直しても比較先は新しくならない。canonical を毎回読むことで新鮮さが保たれるのは **V1 と V2 だけ**である。

  拒否側で影響を受けないのは V1 / V2 に限られる。同事故を検算すると、6 試合中 5 試合はイベント合計 34–32 が実スコア（38–47 / 31–33 / 24–39 / 45–21 / 27–10）と食い違うため **V1 で拒否される**。V3 / V4 の警告が出ないだけで汚染は入らない
