...
`D006` で決定した通り、MVP ローンチ対象は 2026 年 Rugby Championship（8〜10 月、オールブラックス／ワラビーズ／スプリングボクス／プーマスの 4 か国対抗）です。`p1-content-pipeline.md` の 5 段階パイプラインは `matches` / `match_events` / `match_raw_data` が十分に埋まっている前提で動くため、本仕様書ではこれらを実際に Rugby Championship のデータで埋める取り込みを対象にします。
...
対象:
- 競技（`competitions`）と 4 代表チーム（`teams`）の静的シード（マイグレーションまたは TypeScript シードスクリプト）
...
- CRON_SECRET による認可
- Rugby Championship 2026 固有のパーサー（1 ソース 1 ファイル）
...
- 管理画面・再取り込み UI
- Rugby Championship 以外の大会（将来の大会は同じ取り込み層のパーサー追加で対応）
- 再スケジュール（試合の延期対応）: 本 PR では kickoff の update のみで OK、履歴管理しない
- 実ネットワークを叩くテスト

## データソース選定

MVP は **Wikipedia の「2026 Rugby Championship」記事**を一次ソースとする。

選定理由:
- コンテンツが CC-BY-SA ライセンスで再利用可能（`match_raw_data` への保存 + LLM 書き換えの双方に法的懸念が小さい）
- フィクスチャー表・結果表が `wikitable` クラスの HTML で構造化されており、`cheerio` でパース容易
- `robots.txt` で `/wiki/` 配下のクロールは許可されている（Codex は実装時に再確認する）
- 公式団体（World Rugby）のサイトより変動が少なく、MVP 向けに安定
- 試合終了後に `Match X` 節にスタッツや主要イベントが記載される

想定 URL: `https://en.wikipedia.org/wiki/2026_Rugby_Championship`（正式タイトルが異なる場合は Codex が確認）
...
insert into competitions (slug, name, country, season, start_date, end_date) values
  ('rugby-championship-2026', 'Rugby Championship 2026', null, '2026', '2026-08-01', '2026-10-15');
```
...
insert into teams (slug, name, short_code, country) values
  ('all-blacks',  'All Blacks',  'NZL', 'NZL'),
  ('wallabies',   'Wallabies',   'AUS', 'AUS'),
  ('springboks',  'Springboks',  'ZAF', 'ZAF'),
  ('pumas',       'Pumas',       'ARG', 'ARG');
```

### `competition_teams`

上記 4 チームを `rugby-championship-2026` に紐付ける。

シードは新規マイグレーションファイル `<ts>_seed_rugby_championship_2026.sql` に書く。Phase 1 専用データで再利用しないため、`supabase/seed.sql` ではなくマイグレーションに入れる（`p1-data-model.md` の方針と整合）。
...
/lib/ingestion
  /sources
    wikipedia-rc-2026.ts    — Wikipedia の 2026 RC 記事パーサー
...
{
  "status": "ok",
  "competition": "rugby-championship-2026",
...
Codex は以下の構造を前提にパーサーを実装する。構造が異なる場合は実装前に Owner に確認する。

- フィクスチャー表は `table.wikitable` で、列は「Date / Home / Away / Score / Venue」前後
- 日付は "5 August 2026" / "Saturday, 5 August 2026" 等のフォーマットが混在し得る。`date-fns` の parse を使うか、正規表現で揃える
- kickoff 時刻が記載されていない場合は当該日の 00:00 UTC で保存し、`matches.status = 'scheduled'` のまま据え置く
- スコアが記載されていれば `status = 'finished'`、`home_score` / `away_score` を update
- 主要イベント（トライ・カード）の抽出は本 PR では試みず、`match_events` への insert は 0 件で OK。スコアのみ確実に取得することを優先

**スコープ調整**: `match_events` の充実は後続仕様書で扱う。本 PR は「日程・対戦カード・スコア」までを確実に動かす。

## テスト戦略

- `tests/ingestion/wikipedia-rc-2026.test.ts`: 固定の HTML フィクスチャ（`tests/fixtures/wikipedia-rc-2026.html` に保存）をパースし、期待される試合配列が返ることを検証
...
- [ ] `supabase db reset` 後、`competitions` / `teams` / `competition_teams` にシードデータが入っている（RC 2026 + 4 チーム）
- [ ] `tests/fixtures/wikipedia-rc-2026.html` を入力として fixtures パーサーが 12 試合を返す
...
1. Wikipedia 記事のタイトルが `2026_Rugby_Championship` とは異なる場合（例: `2026 The Rugby Championship`、スポンサー名混在）の最終決定 — 実装開始時点で Codex が Wikipedia で確認し、Owner に報告
2. kickoff 時刻が Wikipedia に記載されていない場合、Rugby Championship の公式サイトから補完するか、`00:00 UTC` で据え置くか（本 PR は据え置きを採用、必要に応じて後続 PR で改善）
