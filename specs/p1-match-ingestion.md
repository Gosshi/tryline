...
想定 URL: `https://en.wikipedia.org/wiki/2027_Six_Nations_Championship`（正式タイトルが異なる場合は Codex が確認）
...
- 日付は "6 February 2027" / "Saturday, 6 February 2027" 等のフォーマットが混在し得る。`date-fns` の parse を使うか、正規表現で揃える
...
- `tests/ingestion/wikipedia-six-nations-2027.test.ts`: 固定の HTML フィクスチャ（`tests/fixtures/wikipedia-six-nations-2027.html` に保存）をパースし、期待される試合配列が返ることを検証
...
- [ ] `tests/fixtures/wikipedia-six-nations-2027.html` を入力として fixtures パーサーが 15 試合を返す
...
1. Wikipedia 記事のタイトルが `2027_Six_Nations_Championship` とは異なる場合（例: スポンサー名混在）の最終決定 — 実装開始時点で Codex が Wikipedia で確認し、Owner に報告
2. kickoff 時刻が Wikipedia に記載されていない場合、`00:00 UTC` で据え置くか（本 PR は据え置きを採用、必要に応じて後続 PR で改善）
