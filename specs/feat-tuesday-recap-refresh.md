...
24. **ヒアドキュメントの終端が、YAML の dedent 後に桁0にある。** ブロックスカラーは基準インデントしか除去しないため、`if` に合わせて字下げすると終端として認識されず、以降の行が本文に吸い込まれる
25. **`workflow_dispatch` を実際に実行し、`Target match count` が0でないことをログで確認している**（PR 本文に実行 URL を貼る）

## 検証方法

**lint・tsc・test・build はどれも YAML を実行しません。** ワークフローの不具合はこれらをすべて通過します。

**`workflow_dispatch` で必ず1回走らせてください。** 試合が1件以上入る日付範囲を指定し、`Target match count` が0でないことを確認します。

**ここで0件が出たら、対象解決のロジックが壊れています。** 2026-09-05 に、ヒアドキュメントの終端の字下げで `else` 側の処理ごと消え、月曜の実行が静かに何もしなくなる不具合が実際に発生しました。

**実行は LLM を呼びます。Owner の承認を得てから行ってください。**
