...
**すべて生 HTML に対して検証すること。ブラウザで描画された DOM を見ても意味がない**（ハイドレーション後は元から表示されている）。

> **プレビュー環境では `curl` が使えない。** Vercel SSO のログインページへリダイレクトされ、`/matches/` リンクが0件に見える。**「SSRが壊れている」と誤判定する罠。** 認証済みブラウザのコンテキストから `fetch` すること:
>
> ```js
> const html = await fetch(location.href, { credentials: "include" }).then((r) => r.text());
> [...new Set(html.match(/href="\/matches\/[a-f0-9-]{36}"/g) || [])].length;
> ```
>
> 本番（`www.trylinerugby.com`）では `curl -sL` がそのまま使える。
...
6. **`/c/rwc/2027`** の HTML にフィルタタブ（`[role="tab"]`）と試合カードの**両方**が含まれる（フィルタが必要な大会でも SSR が壊れないこと）

> **2026-09-01 訂正**: 当初この条件と条件13を `/c/nations-championship/2026` で書いていたが、**`fix-hub-preseason-standings-and-round-filter.md`（PR #746、マージ済み）で NC にプールフィルタは出さないことが確定した**（全試合が北半球チーム 対 南半球チームで、1試合が1つのプールに属さないため）。フィルタを持つのは RWC のみ。
...
13. **`/c/rwc/2027`** でフィルタタブを選ぶと絞り込みが効く（PR #749 実測: プールB で 38 → 8 件、URL が `?round=pool-b`）
14. 閉じている節の試合カードが `display:none` 相当で隠れ、スクリーンリーダーに露出しない
15. **`/c/urc/2025-26` の転送量（Brotli 圧縮後）が 60KB を超えない**

> **2026-09-01 訂正**: 当初この条件を「HTML 400KB 以下」としていたが、**非圧縮サイズを測っていた誤った指標だった**。実測（PR #749）:
>
> | | 非圧縮 | **転送量（Brotli）** |
> |---|---|---|
> | 修正前 | 285KB | **30KB** |
> | 修正後 | 834KB | **49KB** |
>
> マークアップが反復的で圧縮効率が18:1 と高いため、**非圧縮サイズは利用者のコストを表していない**。+19KB で150本のクロール可能リンクが得られるのは妥当なトレードオフ。計測は `PerformanceNavigationTiming.encodedBodySize` で行う。
...
1. **閉じている節も含め全カードを HTML に出すか、開いている節だけにするか。** 本 spec は「全カード」を選んだ。理由は (a) データが既に payload に入っており転送増が小さい、(b) 150本の内部リンクこそが主目的、(c) 現行実装が既に「全カードを描画して CSS で隠す」形で挙動差分が出ない。~~ただし受け入れ条件 15（400KB）を超えるなら〜~~ → **決着（2026-09-01）。全カードのまま。** 実測で転送量は 30KB → 49KB（Brotli）に留まり、落とす必要はなかった。ラウンドハブは既に正常に SSR されているため、リンクだけでもクロール経路は確保できる
...
4. **`/c/rwc/2027` が専用ページではなく汎用シーズンページで配信されている**（2026-09-01 発見、本 spec とは無関係の既存問題）。`loadFamilies()`（`lib/db/queries/competitions.ts:298-307`）が全 family を除外なしで返すため、汎用ルートの `generateStaticParams` が `/c/rwc/2027` を生成し、専用の静的ルート `app/c/rwc/2027/page.tsx` と競合していると見られる。結果として専用ページの「開催都市・会場」等が表示されない。**RWC 2027 は本命大会のため別 spec で扱う**
