...
### 「伝説の名場面・選手」セクションのリダイレクトが必須

> **2026-08-18 追記。** 初版はこの点を書き落としていた（spec 側の抜け。PR #703 のレビューで判明）。

`searchContext`（41-49 行）は Exa をこう検索する。

```ts
`${nameJa} ラグビー 歴史 有名試合 伝説的選手 エピソード`
```

**`nameJa`「グレイテスト・ライバルリー・ツアー」は 2026 年のこのツアー用に付いた呼称**であり、日本語で「歴史」「伝説的選手」を検索してもほぼ何も返らない。

一方、プロンプトの必須セクション 3（76 行）は「**年号・スコア・具体的な状況**を盛り込んで 3〜5 つ」を要求し、厳守事項（81 行）は「**参照情報に載っていない具体的なスコアや年号は書かない**」を要求する。**参照情報が空だとこの 2 つは両立せず、捏造に倒れるリスクがある。**

既存の 2 件は、**context でセクションの書き先を明示的にリダイレクト**して同じ問題を回避している。

- `nations-championship`（2026 新設）: 「「伝説の名場面・選手」セクションでは、**参加12カ国の代表チームで**語り継がれる歴史的な名場面・名選手のエピソードを書くこと」
- `autumn-nations`: 「**見どころセクションでは**その意義（…）を説明すること」

**`greatest-rivalry` にも同じリダイレクトを入れること。** ツアー自体は新しいが、**オールブラックス対スプリングボクスのライバル関係には 100 年以上の歴史があり**、そちらへ向ければ書ける。

context に以下に相当する指示を含めること（文言は Codex の判断でよい）:

> 「伝説の名場面・選手」セクションでは、オールブラックスとスプリングボクスの 100 年以上にわたるライバル関係で語り継がれる名場面・名選手のエピソードを書くこと。**このツアー自体は 2026 年に始まったばかりなので、ツアーの歴史ではなく両国の対戦史を扱う。**
...
5b. **`context` に「伝説の名場面・選手」セクションのリダイレクト指示が含まれている**（ツアーの歴史ではなく、オールブラックス対スプリングボクスの対戦史を扱わせる。上記「リダイレクトが必須」参照）
