...
2. **共有の基本形 = 画像＋URL 同時**（Stories への共有はシート経由の画像共有であり、リンクスタンプは手動になる点は了解済み）
   - **v1 実装上の制限（2026-07-18 追記・Owner 承認）**: v1 の共有は「画像 URL のリンク共有」であり、画像ファイルの添付ではない。X・LINE 等はリンクプレビューで画像が表示されるが、**Instagram Stories はリンク共有を受け付けないため直接貼り付け不可**（ユーザーが画像を保存して手動貼り付け）。画像ファイル共有化（リモート画像をローカル保存→file URI 共有、`expo-file-system` の依存追加が必要）は Phase 2 の小 spec 候補とし、TestFlight での実機検証後に判断する
