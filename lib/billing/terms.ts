...
export function createBillingTerms({
  ...
  if (trialDays > 0) {
    ...
    return {
      ...
      pricingDescription: `見逃した海外ラグビーの試合を日本語レビューと試合について質問できる機能で深く追える Tryline Premium。${trialHeroLabel}、その後 ${monthlyPriceLabel}。`,
      pricingSummary: `${trialHeroLabel} · その後 ${monthlyPriceLabel} · いつでもキャンセル可能 · Stripe 決済`,
      serviceProvisionTiming: "無料トライアル開始時から直ちに利用可能",
      trialDays,
      trialFaqAnswer: `はい。初回登録時に ${trialDays} 日間の無料トライアルをご利用いただけます。トライアル期間中は日本語レビュー全文・試合について質問できる機能を含むすべての Premium 機能をお使いいただけます。トライアル終了後は自動的に ${monthlyPriceLabel}の課金が始まります。期間中はいつでもキャンセル可能です。`,
  ...
  return {
    ...
    pricingDescription: `見逃した海外ラグビーの試合を日本語レビューと試合について質問できる機能で深く追える Tryline Premium。${monthlyPriceLabel}。`,
